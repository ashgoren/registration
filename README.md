...
> and use ngrok to create tunnel to your local server for testing.
...
3. Click **Add Webhook** and enter your webhook URL.
    - For local testing, use ngrok (`ngrok http 5001`) to create a local tunnel, then use URL like this: `https://<ngrok-url>/<PROJECT_ID>/<REGION>/paypalWebhook`
...
5. Click **Save**.
6. Copy the webhook ID generated by PayPal into `functions/.env` as `PAYPAL_WEBHOOK_ID`.
...
2. Copy the webhook secret generated by Stripe into `functions/.env` as `STRIPE_WEBHOOK_SECRET`.
(Put production secret in `functions/.env.<PROJECT_ID>` and test secret in `functions/.env.local`.)
...
- Set sandbox mode to false in `functions/.env` (or `/functions/.env.local` and `/functions/.env.<PROJECT_ID>`)
- Disable _enforceAppCheck_ in `functions/.env` if needed
